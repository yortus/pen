...
const parse = (() => {
    ...
    function id1() {
        ...
        if (id4()) { if (OUT !== undefined) out.push(OUT) } else return setState(stateₒ), false;
    ...
    function id5() {
        let stateₒ = getState();
        let out = [];
        if (id6()) { if (OUT !== undefined) out.push(OUT) } else return setState(stateₒ), false;
        if (id7()) { if (OUT !== undefined) out.push(OUT) } else return setState(stateₒ), false;
        OUT = concatAll(out);
        return true;
    }
